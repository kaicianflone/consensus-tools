...
function votesCastSh(): string {
  return [
    ...
    'shift || true',
    '',
    '# Supports either positional args:',
    '#   votes_cast.sh <jobId> SUBMISSION <submissionId> [weight]',
    '#   votes_cast.sh <jobId> CHOICE <choiceKey> [weight]',
    '# Or flag args:',
    '#   votes_cast.sh <jobId> --submission <id> [--weight <n>]',
    '#   votes_cast.sh <jobId> --choice <key> [--weight <n>]',
    '',
    'TARGET_TYPE=""',
    'TARGET_ID=""',
    'WEIGHT="1"',
    ...
    '  echo "   or: votes_cast.sh <jobId> CHOICE <choiceKey> [weight]" >&2',
    '  echo "   or: votes_cast.sh <jobId> --submission <id> [--weight <n>]" >&2',
    '   echo "   or: votes_cast.sh <jobId> --choice <key> [--weight <n>]" >&2',
    '  exit 2',
    'fi',
    '',
    'MODE="$(mode)"',
...
function resolveSh(): string {
  return [
    ...
    '  python3 - <<\'PY\' "$JOB_ID" "$dir" "$result_path"',
    'import json,glob,sys,os',
    'from datetime import datetime, timezone',
    'job_id=sys.argv[1]; d=sys.argv[2]; out_path=sys.argv[3]',
    'subs=[]',
    ...
    'PY',
    '',
    '  # Update job status to RESOLVED (best-effort)',
    '  job_path="$(job_file "$JOB_ID")"',
    '  python3 - <<\'PY\' "$job_path" "$result_path" || true',
    'import json,sys,os',
    'job_path=sys.argv[1]; result_path=sys.argv[2]',
    'with open(job_path,"r") as f: job=json.load(f)',
    'with open(result_path,"r") as f: result=json.load(f)',
    'job["status"]="RESOLVED"',
    'job["resolvedAt"]=result.get("resolvedAt")',
    'job["winnerSubmissionId"]=result.get("selectedSubmissionId")',
    'with open(job_path,"w") as f: f.write(json.dumps(job, indent=2))',
    'PY',
    '',
    '  exit 0',
    'fi',
